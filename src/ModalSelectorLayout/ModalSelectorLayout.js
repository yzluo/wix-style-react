...
const DEFAULT_EMPTY = (
  <div className={css.defaultEmptyStateWrapper}>
    <Text appearance="T1">{"You don't have any items"}</Text>
  </div>
);
...
export default class ModalSelectorLayout extends WixComponent {
            ...
            <div className={css.subheaderWrapper}>
              {subtitle && (
                <div className={css.subtitleWrapper}>
                  <Text appearance="T1" dataHook={dataHooks.subtitle}>
                    {subtitle}
                  </Text>
                </div>
              )}
            ...
            <Selector
              ...
              extraNode={
                item.extraNode ? (
                  ...
                  <Text appearance="T1.1">{item.extraText}</Text>
                )
              }
  ...
  _renderFooter = () => {
    const { isLoaded, selectedItems } = this.state;
  ...
  _renderFooterSelector = () => {
    ...
    return (
      ...
      >
        <Text appearance="T2">{` ${text} (${number})`}</Text>
      </Checkbox>
    );
  };
}
