...
const byHook = (wrapper, hook) =>
  wrapper.querySelector(`[data-hook="${hook}"]`);

const statsWidgetDriverFactory = ({ element, wrapper, component }) => {
  const getStatistic = index =>
    byHook(element, 'stats-widget-content-wrapper').childNodes[index];
  ...
  const driver = {
    ...
    isStatisticsContentExists: () =>
      !!byHook(element, 'stats-widget-content-wrapper'),

    isEmptyStateExists: () => !!byHook(element, 'stats-widget-empty-state'),

    getStatisticTitle: index =>
      byHook(getStatistic(index), 'statistics-item-title').textContent,

    getStatisticSubTitle: index =>
      byHook(getStatistic(index), 'statistics-item-subtitle').textContent,

    getStatisticPercentValue: index =>
      byHook(getStatistic(index), 'percent-value').textContent,

    getStatisticPercentClass: index =>
      byHook(getStatistic(index), 'percent-wrapper').className,
