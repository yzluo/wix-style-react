...
class StatsWidget extends WixComponent {
      ...
      <Text appearance="H3">
        <div
          className={classnames(
            styles.percents,
            { [styles.isNegative]: percent < 0 },
            { [styles.isPositive]: percent > 0 },
          )}
          data-hook="percent-wrapper"
        >
          <span className={classnames(styles.percentArrow)}>
            <SortByArrowUp />
          </span>
  ...
  _renderColumn(statistics, index) {
    return (
      ...
      >
        <Text dataHook="statistics-item-title" appearance="H1">
          {statistics.title}
        </Text>
        ...
        {typeof statistics.percent === 'number' &&
          this._renderPercentage(statistics.percent)}
      </div>
    );
  }
