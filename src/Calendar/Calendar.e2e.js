...
describe('Calendar', () => {
  const storyUrl = ({selectedDays}) => {
    ...
    return `${baseUrl}&selectedDays=${global.encodeURIComponent(JSON.stringify(selectedDays))}`;
  };
  const dataHook = 'calendar';
  const driver = calendarTestkitFactory({dataHook});

  eyes.it('should correctly render when selectedDays is a single date', async () => {
  ...
  eyes.it('should correctly render when selectedDays is a date range', async () => {
