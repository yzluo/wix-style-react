...
export default class LanguagePicker extends WixComponent {
  createDividedChildren(availableOptions) {
    const dividerChild = React.cloneElement(this.props.children[0], {
      children: '-',
      languageKey: '-',
      linkTo: '-',
    });
    ...
    dividedChildren.forEach((_, index) =>
      dividedChildren.splice(index * 2 + 1, 0, dividerChild),
    );
    dividedChildren.pop();
  ...
  createIconWithOptionsOptions(children) {
    const availableOptions = children.filter(
      opt => opt.props.languageKey !== this.props.selectedId,
    );
...
LanguagePicker.propTypes = {
  ...
  children: PropTypes.arrayOf((propValue, key) => {
    if (propValue[key].type !== LanguagePicker.Option) {
      return new Error(
        `LanguagePicker: Invalid Prop children was given. Validation failed on child number ${key}`,
      );
    }
  }).isRequired,
};
